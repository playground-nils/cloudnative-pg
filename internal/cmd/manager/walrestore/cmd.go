...
//
// Success and the sentinel errors that carry their own PostgreSQL-visible
// semantics (WAL not found, no backup configured, end-of-wal-stream,
// external cluster missing) are final. ErrRetryTimeoutReached is also final
// because it is generated by the loop itself once the budget is exhausted.
...
func isTransientRestoreError(err error) bool {
	...
	switch {
	case errors.Is(err, barmanRestorer.ErrWALNotFound),
		errors.Is(err, ErrNoBackupConfigured),
		errors.Is(err, ErrEndOfWALStreamReached),
		errors.Is(err, ErrExternalClusterNotFound),
		errors.Is(err, ErrRetryTimeoutReached):
		return false
	}
	return true
}
...
// into account.
//
// The interesting case is "no barman configured" combined with a transient
// plugin error: returning ErrNoBackupConfigured here would short-circuit the
// retry loop and let PostgreSQL promote on an incomplete archive. Returning
// the wrapped plugin error instead keeps the retry loop alive.
func resolveNoBarmanError(
	...
	if pluginKind == restoreErrorTransient {
		return fmt.Errorf("transient plugin error, no barman-cloud fallback configured: %w", pluginErr)
	}
...
// reported earlier.
//
// When the plugin had a transient failure but barman now reports "WAL not
// found", the answer is ambiguous: a flaky plugin may have actually had the
// WAL, and a fresh attempt could succeed. We deliberately do not wrap
// barman's ErrWALNotFound with %w here — that would defeat the
// isTransientRestoreError sentinel check upstream and the loop would not
// retry.
func combineBarmanFailureWithPluginContext(
	...
	if pluginKind == restoreErrorTransient && classifyBarmanError(barmanErr) == restoreErrorNotFound {
		return fmt.Errorf(
			"plugin reported transient error (%v), barman-cloud says WAL not found (%v); will retry",
			pluginErr, barmanErr)
	}
	return barmanErr
}
